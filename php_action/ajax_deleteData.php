...
if (isset($_REQUEST['delete_bymanually'])) {
	...
	if ($table == "vouchers") {
		...
		// First, delete from the `purchase_items` table
		$delete_purchase_items = mysqli_query($dbc, "DELETE FROM purchase_item WHERE purchase_id='$id'");

		if ($delete_purchase_items) {
			// Then, delete from the `purchase` table
			$delete_purchase = mysqli_query($dbc, "DELETE FROM purchase WHERE $row='$id'");
